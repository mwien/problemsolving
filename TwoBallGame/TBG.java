...
public class TBG {
	...
	for(int c = 0; c < C; c++) {
	    ...
	    Point[] imps  = new Point[4];
	    for(int n = 0; n < N; n++) {
		String[] in = br.readLine().split(" ");
		points[n] = new Point(Double.parseDouble(in[0]), Double.parseDouble(in[1]));
		if(n < 4) imps[n] = points[n];
	    }
	    ...
	    for(int i = 0; i < points.length; i++) {
		for(int j = 0; j < 4; j++) {
		    if(points[i].x == imps[j].x && points[i].y == imps[j].y){
			inds[j] = i;
		    }
		}
	    }
	    ...
	    int add = points.length - inds[0];
	    for(int i = 0; i < 4; i++) {
		inds[i] = (inds[i] + add) % points.length;
	    }
	    if((inds[2] < inds[1] && inds[1] < inds[3]) || (inds[3] < inds[1] && inds[1] < inds[2]) && !flag) System.out.println("IMPOSSIBLE");
...
class Point implements Comparable<Point> {
	...
	if(Calc.crossProd(a, b) == 0 && Calc.dotProd(a, b) > 0) return Double.compare(Calc.dotProd(a, a), Calc.dotProd(b, b));//if same angle, sort by dist
	...
	return Double.compare(0, Calc.crossProd(a, b)); //return negative value if cp larger than zero
    }
}
